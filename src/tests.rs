...
impl IntegrationTestRunner {
    pub fn new(ctx: SessionContext, relative_path: &str) -> Self {
        Self { ctx, relative_path: relative_path.into() }
    }
}
...
async fn get_execution_plan_from_file(file_name: &str) -> Result<Vec<Arc<dyn ExecutionPlan>>, Box<dyn std::error::Error>> {
    let ctx = SessionContext::new();


    sqllogictest::Runner::new(|| async {
        Ok(IntegrationTestRunner::new(
            SessionContext::new(),
            file_name,
        ))
    });

    let sql_statements: Vec<Record<DFColumnType>> = sqllogictest::parse_file(file_name)
