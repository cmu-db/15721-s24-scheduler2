...
use composable_database::{
    AbortQueryArgs, AbortQueryRet, NewTaskPlan, NotifyTaskStateArgs, NotifyTaskStateRet, notify_task_state_args,
    QueryInfo, QueryJobStatusArgs, QueryJobStatusRet, QueryStatus, ScheduleQueryArgs, ScheduleQueryRet
};
...
impl SchedulerApi for SchedulerService {
    ...
    async fn schedule_query(
        ...
        } = request.into_inner()
        {
            ...
            let plan = deserialize_physical_plan(physical_plan.as_slice()).await.unwrap();
    ...
    async fn notify_task_state(
        ...
        Err(Status::invalid_argument("notify_task_state not implemented."))
    }
}
